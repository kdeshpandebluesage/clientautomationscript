...
public class TwoBorrRefiWillianBrennanLoanLOPSupremeTest {
...
//        List<Map<String, String>> data = ExcelUtil.readExcel("src/main/resources/testdata.xlsx");
     //   page.click("//ion-button[contains(@class, 'login') and contains(@class, 'button')]");
        ...
        for (Map<String, String> row : data) {
            new LoanPurposePage(page).refinanceloanpurposeform(row.get("LoanPurposePhoneNumber"));
        }
...
//        }
        for (Map<String, String> row : data) {
            new ApplicationInfoSUPPage(page).applicationInformation(row.get("ApplicationInfo2BrPrimaryApplicantFirstName"), row.get("String ApplicationInfo2BrPrimaryApplicantMiddleName"),row.get("ApplicationInfo2BrPrimaryApplicantLastName"),
        ...
        for (Map<String, String> row : data) {
            new ApplicationInfoSecondaryPage(page).applicationSecondaryInformation(row.get("ApplicationInfo2BrSecondaryApplicantFirstName"), row.get("String ApplicationInfo2BrSecondaryApplicantMiddleName"), row.get("ApplicationInfo2BrSecondaryApplicantLastName"),
