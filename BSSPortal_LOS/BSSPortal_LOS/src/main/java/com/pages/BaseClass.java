...
//     return listElems;
    ...
    public void uploadFile(String selector, String filePath) {
        ...
        /**
         * Generic method to handle file uploads triggered by clicking a UI element (Button/Icon).
         * Use this when 'setInputFiles' fails because the input is hidden or styled purely with JavaScript.
         *
         * @param page The current Playwright Page instance.
         * @param triggerSelector The selector for the button/icon that opens the file dialog.
         * @param filePath The absolute path to the file you want to upload.
         */
        public  void uploadViaFileChooser(Page page, String triggerSelector, String filePath) {
            try {
                // 1. Wait for the file chooser event to fire after clicking the trigger
                ...
                throw new RuntimeException("Failed to upload file using trigger: " + triggerSelector, e);
            }
        }
...
// In BaseClass.java
    ...
    public static void selectDropdownOption(Page page, String dropdownTriggerSelector, String optionXPath) {
        // 1. Trigger the dropdown once
        page.locator(dropdownTriggerSelector).click(new Locator.ClickOptions().setForce(true));

        // 2. Combine the visible container filter with your passed XPath
        // This ensures we only look inside the physically visible dropdown
        String finalXpath = "//div[contains(@class, 'x-boundlist') and not(contains(@style, 'display: none'))]" + optionXPath;
        Locator option = page.locator(finalXpath).last();
        ...
        try {
            ...
            // 4. Dispatch mousedown (ExtJS specific) and click
            option.dispatchEvent("mousedown");
            option.click(new Locator.ClickOptions().setForce(true));

            System.out.println("Successfully selected option using XPath: " + optionXPath);
        } catch (Exception e) {
            System.out.println("Standard selection failed, trying JS fallback for: " + optionXPath);
            // JS Fallback using your specific XPath inside the visible container
            page.evaluate("xpath => { " +
    ...
    public String pluckExtJsValue(String storeId, String searchField, String searchValue, String pluckField) {
        ...
        java.util.ArrayList<String> result = (java.util.ArrayList<String>) page.evaluate(script);
    ...
    public String getBusinessDateFromStart(String startDateStr, int weeksToAdd) {
        return calculateBusinessDate(startDateStr, weeksToAdd * 7, 0, 0);
    }

    // 2. The "From Today" method (your original method refactored)
    public String addBusinessDate(int days, int months, int years) {
        DateFormat df = new SimpleDateFormat("MM/dd/yyyy");
        String today = df.format(new Date());
        return calculateBusinessDate(today, days, months, years);
    }

    // 3. The Private Engine (Common logic for both)
    private String calculateBusinessDate(String startDateStr, int days, int months, int years) {
        DateFormat df = new SimpleDateFormat("MM/dd/yyyy");
        Calendar c = Calendar.getInstance();
        ...
        Set<String> holidays = new HashSet<>(Arrays.asList(
                "01/01/2026", "01/19/2026", "02/16/2026", "05/25/2026",
                "06/19/2026", "07/03/2026", "09/07/2026", "10/12/2026",
                "11/11/2026", "11/26/2026", "12/25/2026"
        ));
        ...
        while (true) {
            ...
            if (dayOfWeek == Calendar.SATURDAY || dayOfWeek == Calendar.SUNDAY || holidays.contains(formatted)) {
    ...
    /**
     * Universal waiter for loading masks.
     * Handles "Please wait", "Loading", "Saving", "Processing", etc.
     */
    public void waitForWorkToComplete() {
        // Regex catches: Loading, loading, Please wait, Saving, Processing
        String loadingRegex = ".*(Loading|Please wait|Saving|Processing|Wait).*";
        Locator loader = page.locator("text=/" + loadingRegex + "/i");

        try {
            // 1. Give the UI 1 second to actually show the mask
            loader.first().waitFor(new Locator.WaitForOptions()
                    .setState(WaitForSelectorState.VISIBLE)
                    .setTimeout(2000));

            System.out.println("Loader detected, waiting for it to finish...");
        } catch (Exception e) {
            // If no loader appears within 1s, the action likely finished instantly
            return;
        }

        // 2. Wait for ALL matching loaders to be hidden
        // We use .last() to track the most recent mask added to the DOM
        loader.last().waitFor(new Locator.WaitForOptions()
                .setState(WaitForSelectorState.HIDDEN)
                .setTimeout(90000)); // Higher timeout (90s) for slow imports/saves

        System.out.println("Work complete. UI is ready.");
    }
    ...
    public static void selectAngularDropdown(Page page, String dropdownTriggerSelector, String optionXPath) {
        // 1. Trigger the dropdown once
        page.locator(dropdownTriggerSelector).click(new Locator.ClickOptions().setForce(true));
