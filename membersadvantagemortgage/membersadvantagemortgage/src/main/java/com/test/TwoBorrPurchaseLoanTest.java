...
public class TwoBorrPurchaseLoanTest {
...
//        List<Map<String, String>> data = ExcelUtil.readExcel("src/main/resources/testdata.xlsx");
     //   page.click("//ion-button[contains(@class, 'login') and contains(@class, 'button')]");
        ...
        for (Map<String, String> row : data) {
            new LoanPurposePage(page).purchaseloanpurposeform(row.get("LoanPurposePhoneNumber"));
        }
        ...
        for (Map<String, String> row : data) {
           // new PurchaseLoanPage(page).purchaseInfoform(row.get("Purchasezipcode"),row.get("Purchasezipcode"),row.get("PurchasepropertyType"),row.get("PurchaseLoanpurchasePrice"),row.get("Purchasedownpayment"));
        }
...
//        }
        for (Map<String, String> row : data) {
            new ApplicationInfoPage(page).TwoBorrApplicationform(row.get("ApplicationInfo2BrPrimaryApplicantFirstName"), row.get("ApplicationInfo2BrPrimaryApplicantLastName"),
        ...
        for (Map<String, String> row : data) {
            new ApplicationInfoSecondaryPage(page).applicationSecondaryInformation(row.get("ApplicationInfo2BrSecondaryApplicantFirstName"), row.get("ApplicationInfo2BrSecondaryApplicantLastName"), row.get("ApplicationInfo2BrSecondaryApplicantLastName"),
