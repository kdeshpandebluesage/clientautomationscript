...
public class TwoBorr_2_Purchase_Portal_TIB {
...
//            new PortalImportPage(page).xmlimportfile(row.get("xmlfilepath") + row.get("xmlfile") + ".xml");
//            new PortalImportPage(page).xmlimportfile(row.get("xmlfile")  + ".xml");
        ...
        for (Map<String, String> row : data) {
            ...
            page.waitForSelector("//div[contains(text(), 'registered successfully')]", new Page.WaitForSelectorOptions().setState(WaitForSelectorState.VISIBLE));
            ...
            page.waitForSelector("//div[contains(@class, 'x-message-box')]//span[text()='OK']", new Page.WaitForSelectorOptions().setState(WaitForSelectorState.VISIBLE));

            page.click("//div[contains(@class, 'x-message-box')]//span[text()='OK']");
        }

        for (Map<String, String> row : data) {
            new PortalCreditPage(page).lopOrderCredit2BR();
        }
        for (Map<String, String> row : data) {
            new PortalAUSPage(page).lopAUSDU();
        }
    }
