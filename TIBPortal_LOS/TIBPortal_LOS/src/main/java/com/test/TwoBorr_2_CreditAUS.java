...
public class TwoBorr_2_CreditAUS {
    ...
    testFormFill() throws IOException, InterruptedException {
        ...
        page.navigate("https://kind-dev.bluesageusa.com/portal/portal/#/login");
...
//        List<Map<String, String>> data = ExcelUtil.readExcel("src/main/resources/testdata.xlsx");
        ...
        for (Map<String, String> row : data) {
            new PortalImportKINDPage(page).importfile();
        }
        for (Map<String, String> row : data) {
            new PortalShortAppKindPage(page).shortApp(row.get("Creditscore"),row.get("AppraisedAmt"),row.get("PropertyType"),row.get("DTI"));

            loanNumber = LoanUtils.getLoanNumber("//div[contains(text(), 'saved successfully')]", page);
            if (loanNumber == null || loanNumber == "") {
                throw new AssertionError("Loan number is null or empty");
            }
            System.out.println(LoanUtils.getLoanNumber("//div[contains(text(), 'saved successfully')]", page));
        }
