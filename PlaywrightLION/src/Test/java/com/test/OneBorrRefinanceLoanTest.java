...
public class OneBorrRefinanceLoanTest {
     ...
     //   page.click("//ion-button[contains(@class, 'login') and contains(@class, 'button')]");
        ...
        for (Map<String, String> row : data) {
            new RefinancePropertyPage(page).RefiPropInfoform(row.get("Refinanceaddress"),row.get("Refinancecity"),
                    row.get("RefinanceState"),row.get("Refinancezipcode"),row.get("RefinancepropertyType"),row.get("RefinancePurpose"),row.get("RefinancePropertyUsed"),row.get("RefinanceCashOutAmt"));
        }
        for (Map<String, String> row : data) {
            new ApplicationInfoPage(page).OneBorrRefiApplicationform(row.get("ApplicationInfo1BrPrimaryApplicantFirstName"),row.get("ApplicationInfo1BrPrimaryApplicantLastName"),
                    row.get("ApplicationInfo1BrPrimaryApplicantBOD"),row.get("ApplicationInfo1BrPrimaryApplicantEmail"),row.get("ApplicationInfo1BrPrimaryApplicantMobNumber"),row.get("ApplicationInfo1BrPrimaryApplicantMovedMonthYear")
                   );
        }
        for (Map<String, String> row : data) {
