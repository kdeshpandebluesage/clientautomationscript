...
public class TwoBorrPurchaseLoanLOPTest {
    ...
    void testFormFill() throws IOException, InterruptedException {
       ...
       // page.navigate("https://lion-bluesage-dev.firebaseapp.com/apply/loan_purpose");
        page.navigate("https://move-qa-lion.bluesageusa.com/apply/login");
...
//        }
        for (Map<String, String> row : data) {
            new ApplicationInfoPage(page).applicationInformation(row.get("ApplicationInfo2BrPrimaryApplicantFirstName"), row.get("ApplicationInfo2BrPrimaryApplicantLastName"),
                    ...
                    row.get("ApplicationInfo2BrPrimaryApplicantStateName"), row.get("ApplicationInfo2BrPrimaryApplicantzipcode"), row.get("ApplicationInfo2BrPrimaryApplicantRent"),row .get ("ApplicationInfo2BrPrimaryApplicantMovedMonthYear")
                     );
        }
        for (Map<String, String> row : data) {
            new ApplicationInfoSecondaryPage(page).applicationSecondaryInformation(row.get("ApplicationInfo2BrSecondaryApplicantFirstName"), row.get("ApplicationInfo2BrSecondaryApplicantLastName"),
        ...
        for (Map<String, String> row : data) {
            new ConsentPage(page).ConsentInfoform2Borr(row.get("borrowerSSN"),row.get("coborrowerSSN") );
        }
//        for (Map<String, String> row : data) {
...
//        }
        for (Map<String, String> row : data) {
            ...
            new SubmitPage(page).SubmitInfoform();
            loanNumber = LoanUtils.getLoanNumber("p:has-text('Loan Number:')",page);
        ...
        page.navigate("https://move-qa.bluesageusa.com/crm/crm/#/login");
...
//            System.out.println(LoanUtils.getLoanNumber("p:has-text('Loan Number:')",page));
        ...
        for (Map<String, String> row : data) {
            new LOPApplicationPage( page).LopApplication();
        }
        for (Map<String, String> row : data) {
            new LOPCreditPage(page).lopOrderCredit2BR();
        }
        for (Map<String, String> row : data) {
        ...
        for (Map<String, String> row : data) {
            new LOPLoanTermPage(page).lopLoantermPurchase(row.get("LOPAppraisedValue"),row.get("LOPCreditscore"),row.get("SettlmentDate"));
        }

        for (Map<String, String> row : data) {
            new LOPPricingPage(page).lopPricing(row.get("logLabel"));
        }
        for (Map<String, String> row : data) {
